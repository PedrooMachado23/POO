...
var AtualizarCliente = /** @class */ (function (_super) {
    ...
    AtualizarCliente.prototype.atualizar = function () {
        ...
        var _loop_1 = function () {
            ...
            var nomeBuscado = this_1.entrada.receberTexto('Digite o nome do cliente para editar ou "sair" para sair: ');
            if (nomeBuscado.toLowerCase() == 'sair') {
                execucao = false;
                return "break";
            }
            var clienteRequisitado = this_1.clientes.find(function (cliente) { return cliente.nome === nomeBuscado; });
            if (clienteRequisitado) {
                var opcao = this_1.entrada.receberTexto('Dado a atualizar (nome, nome social, cpf): ');
                switch (opcao.toLowerCase()) {
                    case 'nome':
                        clienteRequisitado.nome = this_1.entrada.receberTexto('Novo nome: ');
                    case 'nome social':
                        clienteRequisitado.nome = this_1.entrada.receberTexto('Novo nome social: ');
                    case 'cpf':
                        var valorCpf = this_1.entrada.receberTexto('Novo número CPF: ');
                        var emissaoCpf = this_1.entrada.receberTexto('Nova data de emissão: ');
